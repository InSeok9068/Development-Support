 ...
 * @returns csv 형식의 스트링
 */
const toCsvString = (...args: any[]): string => args.join(',');
 ...
 * @returns true/false
 */
const stringCompareNoCase = (src: string | null, target: string | null): boolean => {
...
const isZeroOrNull = (text: number | null) => {
  if (text == null) return true;
 ...
 * 천 콤마 스트링
 */
const toCommaString = (value: string | number | null): string => {
  try {
    if (value == null) {
      return '';
    }
 ...
 * UTF-8 to Base64
 */
const utf8ToBase64 = (utf8: string) => {
 ...
 * 앞에 0으로 채우기
 */
const fillPaddingChar = (str: string | number, length: number, padding?: string) => {
